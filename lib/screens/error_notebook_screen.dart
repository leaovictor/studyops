...
class _ErrorNotebookScreenState extends ConsumerState<ErrorNotebookScreen> {
  ...
  Future<void> _showNoteDialog(BuildContext context, ErrorNote? existing,
      List<Subject> subjects, List<Topic> allTopics) async {
...
class _NoteCardState extends State<_NoteCard> {
  ...
  Color get _subjectColor => widget.subject != null
      ? Color(int.parse('FF${widget.subject!.color.replaceAll('#', '')}',
          radix: 16))
      : AppTheme.primary;
      ...
      return AnimatedContainer(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          InkWell(
              ...
              child: Column(
                  ...
                  children: [
                    Row(children: [
                      Flexible(
                          child: Container(
                              ...
                              child: Text(widget.subject?.name ?? 'Matéria',
                                  style: TextStyle(
                                      color: color,
                                      fontSize: 11,
                                      fontWeight: FontWeight.w600),
                    ...
                    if (widget.note.question.isNotEmpty)
                      Padding(
                          padding: const EdgeInsets.only(top: 10),
                          child: Text(widget.note.question,
                              style: const TextStyle(
          ...
          if (_isExpanded) ...[
            ...
            const SizedBox(height: 12),
            if (widget.note.correctAnswer.isNotEmpty) ...[
              ...
              Text(widget.note.correctAnswer,
                  style: const TextStyle(
                      fontSize: 13,
            ...
            if (widget.note.errorReason.isNotEmpty) ...[
              const Text('POR QUE ERREI?',
                  style: TextStyle(
                      fontSize: 10,
                      fontWeight: FontWeight.bold,
                      color: Colors.grey)),
            ...
            Wrap(spacing: 8, runSpacing: 8, children: [
              FilledButton.icon(
                  onPressed:
                      _isGenerating ? null : () => _generateFlashcards(ref),
                  icon: _isGenerating
                      ? const SizedBox(
                          width: 14,
                          height: 14,
                  ...
                  label: const Text('Gerar Flashcards IA',
                      style: TextStyle(fontSize: 12)),
                  style: FilledButton.styleFrom(
                      backgroundColor: AppTheme.accent,
                      padding: const EdgeInsets.symmetric(horizontal: 12))),
              OutlinedButton.icon(
                  onPressed: _isExplaining ? null : () => _explainWithAI(ref),
                  icon: _isExplaining
                      ? const SizedBox(
                          width: 14,
                          height: 14,
          ...
          Row(children: [
            Expanded(
                child: Text(
                    'Próxima revisão: ${AppDateUtils.displayDate(widget.note.nextReview)}',
  ...
  Future<void> _generateFlashcards(WidgetRef ref) async {
    ...
    try {
      ...
      final cards = await aiService.generateFlashcardsFromError(
          user.uid,
          widget.note.question,
          widget.note.correctAnswer,
          widget.note.errorReason);
      if (!mounted) return;
          ...
          builder: (context) => AlertDialog(
                  ...
                  content: SizedBox(
                      ...
                      child: ListView.separated(
                          ...
                          itemBuilder: (_, i) => Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text('Frente: ${cards[i]['front']}',
                                        style: const TextStyle(
                                            fontWeight: FontWeight.bold,
                                            fontSize: 13)),
                                    const SizedBox(height: 4),
                                    Text('Verso: ${cards[i]['back']}',
                  ...
                  actions: [
                    ...
                    FilledButton(
                        ...
                        child: const Text('Adicionar Todos'))
                  ]));
      if (confirm == true && mounted) {
        final flashcardCtrl = ref.read(flashcardControllerProvider.notifier);
        for (final cardData in cards) {
          await flashcardCtrl.create(Flashcard(
              id: '',
              userId: user.uid,
              goalId: widget.note.goalId,
              subjectId: widget.note.subjectId,
              topicId: widget.note.topicId,
              front: cardData['front']!,
              back: cardData['back']!,
              fsrsCard: fsrs.Card(cardId: 1).toMap(),
              due: DateTime.now(),
              createdAt: DateTime.now()));
        }
        if (mounted)
          ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Flashcards criados!')));
      }
    } catch (e) {
      if (mounted)
      ...
      return Padding(
          ...
          child: SingleChildScrollView(
              child: Column(
                  ...
                  children: [
                Text(widget.existing == null ? 'Novo Erro' : 'Editar Erro',
                    style: TextStyle(
                        fontSize: 18,
                ...
                FilledButton(
                    onPressed: () {
                      ...
                      final note = ErrorNote(
                          ...
                          goalId:
                              widget.existing?.goalId ?? widget.activeGoalId,
                    ...
                    child: Text(
                        widget.existing == null ? 'Salvar Erro' : 'Atualizar')),
              ])));
    });
  }
}
